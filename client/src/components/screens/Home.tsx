...
export default function HomeScreen() {
  ...
  const [userPrompt, setUserPrompt] = useState<string>("Elon");
  ...
  const generateImage = async () => {
    ...
    setErrorState(false);
    await axios.post(
      `${import.meta.env.VITE_APP_REACT_APP_SERVER_URL}/generator`,
      { prompt: userPrompt }
    ).then(({ data }) => {
  ...
  useEffect(() => {
    axios(`${import.meta.env.VITE_APP_REACT_APP_SERVER_URL}/authenticate`, {
  ...
  return (
    <>
      <Head title="Home" />
      <div className="font-sans antialiased text-gray-900 leading-normal tracking-wider bg-cover bg-sm md:bg-md">
          ...
          <div className="max-w-4xl flex items-center h-auto lg:h-screen flex-wrap mx-auto my-32 lg:my-0">
            ...
            <div id="profile" className="w-full lg:w-3/5 rounded-lg lg:rounded-l-lg lg:rounded-r-none shadow-2xl bg-white opacity-75 mx-6 lg:mx-0">
              <div className="p-4 md:p-12 text-center lg:text-left">
                ...
                <p className="pt-8 text-sm">Totally optional short description about your idea, what you do and so on.</p>
                <input type="text" className="mt-5 input input-bordered w-full max-w-xs"
                  onChange={(e) => setUserPrompt(`Elon Musk ${e.target.value}`)} />
            ...
            <div className="w-full lg:w-2/5">
              <img src={logo} className="rounded-none lg:rounded-lg shadow-2xl hidden lg:block" style={{ height: "480px" }} />
            </div>
      ...
      <Modal title={userPrompt} state={loadingState} url={imageUrl} error={errorState} />
    </>
  );
}
